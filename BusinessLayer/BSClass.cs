...
namespace BusinessLayer
{
    public class BSClass
    {
        ...
        {
            int iSuccess = 0;
            string sQry = "insert into TblSO(SupplierId,SOdatetime,ApproverId,Status,Amount,SOPrintno,IsReturn,DelAddress,Refno,Deliverydate) values ('" + sSuppliername + "','" + Podatetime.ToString("yyyy/MM/dd") + "','" + iSelectapprover + "','" + sStatus + "','" + sAmount + "','" + iSOprintMax + "',0,'"+deladdress+"','"+Refno+"','"+ Deliverydate.ToString("yyyy/MM/dd")+ "')";
            iSuccess = dbObj.InlineExecuteNonQuery(sQry);
            return iSuccess;
        }
        ...
        {
            int iSuccess = 0;
            string sQry = "insert into tblTransSo(Soid,Productid,Licenseno,Productname,Dosageform,Strength,Packtype,Packsize,SoQty,Priceperpack,VAT,Totalamount,IsReturn) values (" + Soid + ",'" + Productid + "','" + sLicenseno + "','" + sProductname + "','" + sDosageform + "','" + sStrength + "','" + sPacktype + "','" + sPacksize + "','" + sSoQty + "','" + sPriceperpack + "','" + sVAT + "','" + sTotalamount + "',0)";
            iSuccess = dbObj.InlineExecuteNonQuery(sQry);
            return iSuccess;
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select distinct po.Poid,po.POPrintno from tblPO po, tblTransPo tpo where po.Poid=tpo.Poid and tpo.isgoodreceived<>1 and po.Status='Approved'";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select * from tblGoodReceipts gr INNER JOIN tblProduct p on gr.ProductId=p.ProductID order by GRId DESC";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "SELECT gr.*, r.ReturnPrintno,p.POPrintno FROM tblGoodReceipts gr LEFT JOIN tblReturn r ON r.Returnid = gr.Returnid left join TblPO p on p.Poid=gr.Poid WHERE gr.GRId ='"+iGRid+"'";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        public DataSet SoList_new()
        {
            ...
            string sQry = "SELECT s.soid,s.SOPrintno FROM tblso s order by soid desc";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
